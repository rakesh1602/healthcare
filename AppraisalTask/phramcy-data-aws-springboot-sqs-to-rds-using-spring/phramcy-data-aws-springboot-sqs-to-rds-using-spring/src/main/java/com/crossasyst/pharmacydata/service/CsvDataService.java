...
public class CsvDataService {
    ...
    public void processPayloadFromLambdaFunction(Message message) throws JsonProcessingException {
        log.info("Inside the method processPayloadFromLambdaFunction()");
        log.info("Payload Received from AWS lambda {}", message);

        MessageEntity messageEntity = objectMapper.readValue(message.body(), MessageEntity.class );
        log.info("Message Entity to save {}", messageEntity);
        messageRepository.save(messageEntity);
        log.info("Message saved to database.");
    }
}
